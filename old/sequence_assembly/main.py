...
def loop_kmer(data, kstart, kend, lstart=0, lend=1):
    ...
    res_time = input("\nDo you want a run-time file (y)?: ")
    runtime = []
    num_reads = lend - lstart
    ...
    for i in range(kstart, kend+1):                             # Loop thru k-mer range
        ...
        if(res_align == 'Y' or res_align == 'y'):               # Perform alignment and create corresponding files
            files = eulerian_string(file, str(i))               # Create Eulerian cycle and path
            align_contig(files[0], i)

    if(res_time == 'Y' or res_time == 'y'):
        make_csv(runtime)
...
def main():
    ...
    # Use default file
        ...
        loop_kmer(data, 3, k, lstart, lend)
